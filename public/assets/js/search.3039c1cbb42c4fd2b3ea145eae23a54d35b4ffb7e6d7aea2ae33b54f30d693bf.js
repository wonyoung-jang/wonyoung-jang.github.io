    ...
    if (fuse) {
      ...
      if (results.length !== 0) {
        ...
        for (let item in results) {
          resultSet += `<li class="post-entry"><span class="entry-header">${results[item].item.title}</span><a href="${results[item].item.permalink}" aria-label="${results[item].item.title}"></a></li>`;
        }
